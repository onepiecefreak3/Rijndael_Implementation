...
namespace Rijndael_Implementation
{
    public class Rijndael
    {
        ...
        {
            if (a != 0 && b != 0) //falls a != 0 und b != 0
                return exponents[(logs[a] + logs[b]) % 0xff];
            else
                return 0;
        }

        private byte GDiv(byte a, byte b)
        {
            var expInd = exponents.ToList().FindIndex(x => x == a);
            var logInd = (0x100 + (expInd - logs[b])) % 0xff;

            return logs[logInd];
        }

        byte[] exponents = new byte[] {
